...
async function buildShell(command) {
  ...
  if (command.type === 'shell') {
    fs.writeFileSync(path.resolve(__dirname, `../../../../shell-ui-database/lib/userShell/${command.command}.sh`), command.shell);
  }
  // 创建子脚本的存放目录
...
function deletePackageShell(command) {
  return new Promise(resolve => {
    ...
    shell.rm('-f', path.resolve(__dirname, `../../../../shell-ui-database/lib/userShell/${command.command}.sh`));
    // 删除之前的 shell 子脚本
